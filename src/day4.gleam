...
pub fn run(lines: List(String)) -> #(Int, Int) {
  ...
  let p1_removed = remove_rolls(grid, set.new(), True)
...
fn remove_repeatedly(
  ...
  let new = remove_rolls(grid, removed, False)
  let old_size = set.size(removed)
  case set.size(new) {
    s if old_size == s -> removed
...
fn remove_rolls(
        ...
        case c {
          ...
          _ -> {
            case set.contains(#(x, y), removed) {
              ...
              False -> {
                let roll_count =
                  count_rolls(x, y, height, grid, removed, acc, p1)
                case roll_count <= 4 {
  ...
  set.union(removed, newly_removed)
}
